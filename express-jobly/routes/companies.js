...
const companyFilters=require("../schemas/companyFilters.json");
...
router.get("/", async function (req, res, next) {
  try {
    ...
    const validator = jsonschema.validate(filters, companyFilters);
    if (!validator.valid) {
      const errs = validator.errors.map(e => e.stack);
      throw new BadRequestError(errs);
    }
...
router.patch("/:handle", ensureLoggedInIsAdmin, async function (req, res, next) {
  try {
...
router.delete("/:handle", ensureLoggedInIsAdmin, async function (req, res, next) {
  try {
